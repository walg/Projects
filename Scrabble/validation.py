...
'''CHANGE AWAY FROM CLASS AND INTO FUNCTIONS'''
...
def _check_horizontal(x,y,board):
    '''checks horizontally until an empty space is reached'''
    word = ''
    for z in range(15):
        try:
...
def _check_vertical(x,y,board):
    '''checks vertically until an empty space is reached'''
    word = ''
    for z in range(15):
        try:
...
def execute(board):
    ...
    for y in range(15):
        for x in range(15):
            ...
            if last_check == None and check != None and _check_space(x+1,y,board) != None: #checks current space and one space to the left
